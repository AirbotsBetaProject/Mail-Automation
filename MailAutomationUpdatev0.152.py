...
class mainwindow(object):
    ...
    def  __init__(self):
        ...
        with open('C:\\Users\\USER\\Downloads\\1.csv') as f:
        ...
        for i in range(len(columns['Sample Mail-ID'])):
            ...
            mainwindow.lg.append(columns['Sample Mail-ID'][i])
        time.sleep(3)
        for i in range(len(columns['Sample Name'])):

            mainwindow.lh.append(columns['Sample Name'][i])
        time.sleep(3)
        for i in range(len(columns['Signin Email'])):

            mainwindow.xs.append(columns['Signin Email'][i]) 
        for i in range(len(columns['Signin Password'])):

            mainwindow.ys.append(columns['Signin Password'][i])
        for i in range(len(columns['ID'])):

            mainwindow.vs.append(columns['ID'][i])
        for i in range(len(columns['Receiver Name'])):

            mainwindow.lt.append(columns['Receiver Name'][i])
        time.sleep(3)
        ...
        for i in range(len(columns['Recevier Mail-ID'])):
            if len(columns['Recevier Mail-ID'][i])!=0:
                ...
                mainwindow.ls.append(columns['Recevier Mail-ID'][i])
        time.sleep(3)
...
class Actual(tk.Frame,mainwindow):
    ...
    def __init__(self,parent):
        ...
        self.B1=Button(parent, text=" Start  ",bg="blue",fg="white",command=self.report
            ).grid(row=5,column=1)
...
def messagess(no):
    ...
    if no==1:
        ...
        message = client.api.account.messages.create(to="+919003413055",
                                             from_="+19202800064",
