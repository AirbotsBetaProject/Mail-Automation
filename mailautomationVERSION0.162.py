        ...
        with open('1.csv') as f:
            ...
            for row in reader: 
                for (k,v) in row.items(): 
                    columns[k].append(v) 
        for i in range(len(columns['Sample Mail-ID'])):
            if len(columns['Sample Mail-ID'][i])!=0:

                mainwindow.readlg.append(columns['Sample Mail-ID'][i])
        time.sleep(3)
        for i in range(len(columns['Sample Name'])):
            if len(columns['Sample Name'][i])!=0:

                mainwindow.readlh.append(columns['Sample Name'][i])
        time.sleep(3)
        for i in range(len(columns['Signin Email'])):
            if len(columns['Signin Email'][i])!=0:

                mainwindow.readxs.append(columns['Signin Email'][i]) 
        for i in range(len(columns['Signin Password'])):
            if len(columns['Signin Password'][i])!=0:

                mainwindow.readys.append(columns['Signin Password'][i])
        for i in range(len(columns['ID'])):
            if len(columns['ID'][i])!=0:

                mainwindow.readvs.append(columns['ID'][i])
        for i in range(len(columns['Receiver Name'])):
            if len(columns['Receiver Name'][i])!=0:

                mainwindow.readlt.append(columns['Receiver Name'][i])
        time.sleep(3)
        
        for i in range(len(columns['Recevier Mail-ID'])):
            if len(columns['Recevier Mail-ID'][i])!=0:


                mainwindow.readls.append(columns['Recevier Mail-ID'][i])
        time.sleep(3)
...
class Actual(tk.Frame,mainwindow):
    ...
    def multithreading1(self):
        ...
        Actual.pend=len(mainwindow.ls)-Actual.key
        if Actual.pend==0:
            count=5
            messagess(count)
        if Actual.pend!=0:
    ...
    def multithreading(self):
        ...
        for i in range(bac, len(self.ls)):
            ...
            if i <= 500:
                ...
                print("Contact Developer.... Needs Renovation.")
        solve.pend=len(mainwindow.ls)-solve.key
        if solve.pend==0:
            count=5
            messagessedule(count)
        if solve.pend!=0:
